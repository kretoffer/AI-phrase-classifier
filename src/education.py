...
def educate(data, embedding_matrix, input_layer, hidden_layer, output_layer):
    ...
    for epoch in range(epochs):
        ...
        print(f"Loss {round((e_loss / list(data.values())[0]) * 100, 3)}%")
        print(f"Correct {round((e_correct / list(data.values())[0]) * 100, 3)}%")
