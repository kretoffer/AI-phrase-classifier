...
def start_educate(path_to_project: str):
    ...
    educate(data, embedding_matrix, config["embedding_dim"]**2, config["hidden_layer"], len(config["intents"]), path_to_project, config["activation_method"], config["epochs"]) # type: ignore
