...
from src.shemes import DeletebleEntity, Project

router = APIRouter()


def template_edit_page(
    *components: AnyComponent, name: Optional[str] = None
) -> List[AnyComponent]:
    return [
        ...
        c.Footer(
            ...
            links=[
                c.Link(
                    ...
                    on_click=GoToEvent(
                        url="https://github.com/kretoffer/ai-phrase-classifier"
                    ),
                )
            ],
        ),
        ...
        c.Page(
            components=[  # type: ignore
                ...
                c.Button(
                    ...
                    on_click=GoToEvent(
                        url=f"/api/download-dataset/{project.name}", target="_blank"
                    ),
                ),
                c.Paragraph(text=""),
                c.Form(
                ...
                c.Button(
                    ...
                    on_click=PageEvent(name="open-project-dir"),
                ),
                c.Paragraph(text=""),
                c.Button(
                    text="Delete",
                    named_style="warning",
                    on_click=PageEvent(name="delete-project-modal"),
                ),
                c.Modal(
                        ...
                        c.Form(
                            form_fields=[
                                c.FormFieldInput(
                                    ...
                                    title="Project name",
                                    required=True,
                                    description=f"to delete this project print project name ({project.name})",
                                )
                            ],
                            footer=[],
                            submit_url=f"/api/delete-project/{project.name}",
                            submit_trigger=PageEvent(
                                name="delete-project-modal-submit"
                            ),
                        )
                    ],
                    footer=[
                        c.Button(
                            ...
                            on_click=PageEvent(name="delete-project-modal", clear=True),
                        ),
                        c.Button(
                            text="Delete",
                            on_click=PageEvent(name="delete-project-modal-submit"),
                            named_style="warning",
                        ),
                    ],
...
@router.get(
    "/project/{name}/edit/dataset/view",
    response_model=FastUI,
    response_model_exclude_none=True,
    tags=["fast ui interface"],
)
def view_dataset_page(name: str, request: Request):
    ...
    for id, el in enumerate(dataset["hand-data"]):
        ...
        hand_data.append(
            DatasetHandFull(
                ...
                id=id,
                **{
                    slot["entity"]: sinanimizate(
                        sinonimz, text[slot["start"] : slot["end"]]
                    )
    ...
    columns.append(
        DisplayLookup(
            field="edit",
            on_click=GoToEvent(
                url="$2$1/edit-hand-element/{id}".replace("$1", project.name).replace(
                    "$2", str(request.base_url)
                )
            ),
        )
    )
    hand_data_table = (
        c.Table(data=hand_data, columns=columns)
        if hand_data
        else c.Heading(text="So far, not one phrase", level=3)
    )
    ...
    columns = [DisplayLookup(field="text")]
    columns.extend([DisplayLookup(field=entity) for entity in project.entities])
    ...
    for template_id, data in enumerate(template_dataset):
        content = [
            c.Markdown(
                text=f"Template {template_id}    **{data[0].classification.value}**  "
    ...
    if not template_data_tabels:
        template_data_tabels = [
            c.Heading(text="You haven't added any phrase templates yet.", level=3)
        ]
...
def edit_intents_page(name: str, request: Request):
    ...
    table = (
        c.Heading(text="So far, not one intent", level=2)
        if not intents
        else c.Table(
            data=intents,
            columns=[
                ...
                DisplayLookup(
                    field="delete",
                    on_click=GoToEvent(
                        url="$2api/$1/delete-intent/{name}".replace(
                            "$1", project.name
                        ).replace("$2", str(request.base_url))
                    ),
                ),
            ],
        )
        ...
        c.Modal(
            ...
            body=[
                ...
                c.Form(
                    form_fields=[
                        c.FormFieldInput(
                            name="intent-name", title="Name of intent", required=True
                        )
                    ],
                    footer=[],
            ...
            footer=[
                c.Button(
                    text="Cancel",
                    named_style="secondary",
                    on_click=PageEvent(name="add-intent-modal", clear=True),
                ),
                # c.Button(text="Import prepared", named_style='secondary'),
                c.Button(
                    text="Submit", on_click=PageEvent(name="add-intent-modal-submit")
                ),
            ],
...
def edit_entities_page(name: str, request: Request):
    ...
    table = (
        c.Heading(text="So far, not one entity", level=2)
        if not entities
        else c.Table(
            data=entities,
            columns=[
                ...
                DisplayLookup(
                    field="delete",
                    on_click=GoToEvent(
                        url="$2api/$1/delete-entity/{name}".replace(
                            "$1", project.name
                        ).replace("$2", str(request.base_url))
                    ),
                ),
        ...
        c.Modal(
            ...
            body=[
                c.Form(
                    form_fields=[
                        c.FormFieldInput(
                            name="entity-name", title="Name of entity", required=True
                        )
                    ],
                    footer=[],
                    submit_url=f"/api/add-intent-entity/{project.name}",
            ...
            footer=[
                c.Button(
                    text="Cancel",
                    named_style="secondary",
                    on_click=PageEvent(name="add-entity-modal", clear=True),
                ),
                c.Button(
                    text="Submit", on_click=PageEvent(name="add-entity-modal-submit")
                ),
            ],
        ...
        c.Form(
            ...
            form_fields=[
                c.FormFieldFile(
                    ...
                    required=True,
                    multiple=True,
                    description="Uplad file in json format",
                )
            ],
        ),
        c.Paragraph(text=""),
