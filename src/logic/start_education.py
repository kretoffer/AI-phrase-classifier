...
def start_educate(path_to_project: str):
    ...
    data_for_entity_education, new_educated = start_educate_extractors(project, extractors_data)
...
def start_educate_extractors(project: Project, phrases: dict): 
    ...
    for intent in project.intents:
        ...
        for el in phrases[intent]:
            body = {"entities": [(slot["start"], slot["end"], slot["entity"]) for slot in el["slots"]]}
...
def start_education_threads(classifier_data: tuple, extractors_data: dict, project: Project):
    ...
    for line in queue:
        ...
        for el in line:
            if el == -1:
                ...
                intent = args[el]
                t = multiprocessing.Process(target=educate_entity_extractor, args=(intent, extractors_data[intent], project, f"{projects_dir}/{project.name}"))
            threads.append(t)
            t.start()
