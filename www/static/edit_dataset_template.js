...
async function fetchElementData() {
    try {
        const response = await fetch(`/api/dataset-template-element-data/${project_name}/${element_id}`);
...
function load_phrases(phrases){
    for (const phrase of phrases){
        load_phrase(phrase)
    }
}

function load_phrase(phrase){
    ...
    phrase_line.appendChild(phrase_input);
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    load_phrases(element_data.texts);
    load_slots(element_data.entitys);
});
...
async function submit(){
    ...
    fetch(`/api/update-template-element/${project_name}/${element_id}`, {
        method: "POST",
        headers: {
    ...
    window.location.href = `/web/project/${project_name}/edit/dataset/view#template-data-in-dataset`;
}

function delete_element(){
    fetch(`/api/delete-template-element/${project_name}/${element_id}`, {
        method: 'DELETE'
    })
    .then(response => {
        if (!response.ok){
